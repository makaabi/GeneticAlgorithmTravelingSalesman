...
def geneticAlgorithm(
    ...
    for i in range(200):
        ...
        if gen_number % 10 == 0:
            print("generation "+str(gen_number)+",distance "+str(sorted(population)[0][0]))
...
cities = myfunctions.getCities("cities.txt")
firstPopulation, firstFitest = selectPopulation(cities, POPULATION_SIZE)
solution, genNumber = geneticAlgorithm(
    firstPopulation,
    len(cities),
    TOURNAMENT_SELECTION_SIZE,
    MUTATION_RATE,
...
print("Fittest chromosome distance after training: " + str(solution[0]))
print("Target distance: " + str(TARGET))
print("----------------------------------------------------------------\n")

myfunctions.showMap(cities, solution)
